...
class VQADataProvider:
    ...
    @staticmethod
    def seq_to_char_list(s):
        qc_list = list(s.lower())
        qc_list = filter(lambda x: len(x) > 0, qc_list)
        return qc_list
    ...
    def qc_list_to_vec(self, max_length, q_list):
        """
        Converts a list of chars into a format suitable for the embedding layer.

        Arguments:
        max_length -- the maximum length of a question sequence
        q_list -- a list of chars which are the tokens in the question

        Returns:
        qvec -- A max_length length vector containing one-hot indices for each char
        cvec -- A max_length length sequence continuation indicator vector
        """
        qvec = np.zeros(max_length)
        cvec = np.zeros(max_length)
        for i in xrange(max_length):
            if i < max_length - len(q_list):
                ...
                c = q_list[i-(max_length-len(q_list))]
    ...
    def create_batch(self,qid_list):
        ...
        for i,qid in enumerate(qid_list):
            ...
            q_list = VQADataProvider.seq_to_char_list(q_str)
            t_qvec, t_cvec = self.qc_list_to_vec(self.max_length, q_list)
...
class VQADataProviderLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        top[0].reshape(self.batchsize,config.MAX_CHARS_IN_QUESTION)
        top[1].reshape(self.batchsize,config.MAX_CHARS_IN_QUESTION)
