...
def qlstm(mode, batchsize, T, question_vocab_size):
    ...
    # convolution
    n.word_feature_2 = L.Convolution(n.embed_scale_resh, kernel_h=3, kernel_w=50, stride=1, num_output=256, pad_h=1, pad_w=0, weight_filler=dict(type='xavier')) # N x 256 x ? x 1
    n.word_feature_3 = L.Convolution(n.embed_scale_resh, kernel_h=5, kernel_w=50, stride=1, num_output=256, pad_h=2, pad_w=0, weight_filler=dict(type='xavier'))
    n.word_feature_4 = L.Convolution(n.embed_scale_resh, kernel_h=7, kernel_w=50, stride=1, num_output=256, pad_h=3, pad_w=0, weight_filler=dict(type='xavier'))
    n.word_feature_5 = L.Convolution(n.embed_scale_resh, kernel_h=9, kernel_w=50, stride=1, num_output=256, pad_h=4, pad_w=0, weight_filler=dict(type='xavier'))
    ...
    n.word_vec_2 = L.Pooling(n.word_relu_2, kernel_h=T, kernel_w=1, stride=T, pool=P.Pooling.MAX) # N x 256 x 1 x 1
    n.word_vec_3 = L.Pooling(n.word_relu_3, kernel_h=T, kernel_w=1, stride=T, pool=P.Pooling.MAX)
    n.word_vec_4 = L.Pooling(n.word_relu_4, kernel_h=T, kernel_w=1, stride=T, pool=P.Pooling.MAX)
    ...
    n.concat_vec = L.Concat(*word_vec, concat_param={'axis': 1}) # N x 1024 x 1 x 1
