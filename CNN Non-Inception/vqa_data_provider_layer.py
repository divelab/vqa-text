...
class VQADataProvider:
    ...
    def load_data(data_split_str):
        ...
        '''
        max = 0
        for qid in all_qdic.keys():
            # sequence to list
                q_str = all_qdic[qid]['qstr']
                q_list = VQADataProvider.seq_to_list(q_str)
                if len(q_list) > max:
                    max = len(q_list)
        return max, all_qdic, all_adic
        '''
    ...
    def qlist_to_vec(self, max_length, q_list):
        ...
        for i in xrange(max_length):
            if i < max_length - len(q_list):
                ...
                '''
                # I think the original way is right for RNN but wrong for CNN here. I want to keep the word embedding for unknown word as zero vector.
