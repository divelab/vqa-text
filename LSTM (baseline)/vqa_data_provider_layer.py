...
class VQADataProviderLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        top[0].reshape(config.MAX_WORDS_IN_QUESTION,self.batchsize)
        top[1].reshape(config.MAX_WORDS_IN_QUESTION,self.batchsize)
        top[2].reshape(self.batchsize,2048,14,14)
        top[3].reshape(self.batchsize)
        # top[4].reshape(config.MAX_WORDS_IN_QUESTION,self.batchsize,GLOVE_EMBEDDING_SIZE)
